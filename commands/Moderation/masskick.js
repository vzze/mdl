...
module.exports = {
    name: 'masskick',
    description: 'Works like a usual kick command but you can select more than 1 user.',
    usage: `\`${prefix}masskick\` <User> \n \`${prefix}masskick\` <User1> <User2>...`,
    cooldown: 5,
    premium: "Non-Premium",
    execute(client, message, args) {
        ...
        let op = new MessageEmbed()
            .addField('Successful kicks', 
                "> " + succ.map(t => `\`${t.member.tag}\``)
                    .join(", ")
            )
            .addField('Unsuccessful kicks',
